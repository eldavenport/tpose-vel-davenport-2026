"""
Remake wvel_uvel_div75m_sw_difference.png and uvel_div75m_sw_difference.png
with divergence (and U, V maps) averaged over 0–30 m instead of 0–75 m.
...
def compute_div_30m(ds_run):
    """Horizontal divergence averaged 0–30 m, time-mean (s⁻¹)."""
...
def make_5col(outfile):
    ...
    lev_u30  = sym_levels([
        ds.UVEL.sel(YC=lat_slice,XG=lon_slice,Z=slice(0,-30)).mean(['time','Z']),
        ds_vel.UVEL.sel(YC=lat_slice,XG=lon_slice,Z=slice(0,-30)).mean(['time','Z'])])
    lev_v30  = sym_levels([
        ds.VVEL.sel(YG=lat_slice,XC=lon_slice,Z=slice(0,-30)).mean(['time','Z']),
        ds_vel.VVEL.sel(YG=lat_slice,XC=lon_slice,Z=slice(0,-30)).mean(['time','Z'])])
    ...
    for row, (label, dset, div_da, sw_da) in enumerate(rows_def):
        ...
        # Col 1: UVEL map 0–30 m
        if label == 'Difference':
            d1 = (ds_vel.UVEL.sel(YC=lat_slice,XG=lon_slice,Z=slice(0,-30)).mean(['time','Z']) -
                  ds.UVEL.sel(YC=lat_slice,XG=lon_slice,Z=slice(0,-30)).mean(['time','Z']))
            lev1 = sym_levels([d1])
        else:
            d1 = dset.UVEL.sel(YC=lat_slice,XG=lon_slice,Z=slice(0,-30)).mean(['time','Z'])
            lev1 = lev_u30
        ...
        ax[row,1].set_title(f'{label}, avg U 0–30 m')
        ax[row,1].set_xlabel('' if row<2 else 'Longitude')
        ax[row,1].set_ylabel('Latitude')

        # Col 2: VVEL map 0–30 m
        if label == 'Difference':
            d2 = (ds_vel.VVEL.sel(YG=lat_slice,XC=lon_slice,Z=slice(0,-30)).mean(['time','Z']) -
                  ds.VVEL.sel(YG=lat_slice,XC=lon_slice,Z=slice(0,-30)).mean(['time','Z']))
            lev2 = sym_levels([d2])
        else:
            d2 = dset.VVEL.sel(YG=lat_slice,XC=lon_slice,Z=slice(0,-30)).mean(['time','Z'])
            lev2 = lev_v30
        ...
        ax[row,3].set_title(f'{label}, ∂u/∂x+∂v/∂y, 0–30 m')
...
# ── Run ───────────────────────────────────────────────────────────────────────
make_5col('wvel_uvel_div75m_sw_difference.png')
make_3col('uvel_div75m_sw_difference.png')
